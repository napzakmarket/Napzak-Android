...
class ExploreTracker @Inject constructor(
     ...
     * @param isForSale 판매글 여부
     */
    fun trackStartedChatFromExplore(postId: Long, isForSale: Boolean) {
        analytics.logEvent(
            MixpanelConstants.STARTED_CHAT,
            mapOf(
                POST_ID to postId,
                POST_TYPE to if (isForSale) POST_TYPE_FOR_SALE else POST_TYPE_WANTED,
                USER_ROLE to if (isForSale) USER_ROLE_BUYER else USER_ROLE_SELLER,
            ),
        )
    }
    ...
    companion object {
        ...
        private const val POST_TYPE_FOR_SALE = "for_sale"
        private const val POST_TYPE_WANTED = "wanted"
